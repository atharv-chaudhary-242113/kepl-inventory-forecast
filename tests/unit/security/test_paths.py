...
def test_dotdot_inside_filename_is_not_path_traversal() -> None:
    paths.validate_local_path(Path("..report.xlsx"))


def test_drive_relative_path_rejected() -> None:
...
# pyrefly: ignore [implicit-any-parameter]
def test_size_limit_accepts_small_file(tmp_path) -> None:
...
# pyrefly: ignore [implicit-any-parameter]
def test_size_limit_rejects_oversized_file(tmp_path, monkeypatch) -> None:
...
# pyrefly: ignore [implicit-any-parameter]
def test_size_limit_missing_file_is_data_validation_error(tmp_path) -> None:
    with pytest.raises(DataValidationError):
        paths.enforce_file_size_limits(tmp_path / "nope.csv")


# pyrefly: ignore [implicit-any-parameter]
def test_size_limit_directory_is_data_validation_error(tmp_path) -> None:
    with pytest.raises(DataValidationError):
