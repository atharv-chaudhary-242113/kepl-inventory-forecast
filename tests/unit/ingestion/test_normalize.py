...
def test_forward_fill_accepts_custom_column_name() -> None:
...
def test_normalize_supplier_strips_branch_suffix() -> None:
    df = pl.DataFrame(
...
def test_normalize_supplier_accepts_custom_column_name() -> None:
    df = pl.DataFrame({"vendor": [" ABC (Noida) ", "XYZ"]})
    out = df.with_columns(normalize_supplier_expr("vendor"))["vendor"].to_list()
    assert out == ["ABC", "XYZ"]


def test_normalize_supplier_collapses_branches_to_one_identity() -> None:
